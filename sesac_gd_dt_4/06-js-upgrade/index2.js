...
console.log(arr === copyArr); // false => 복사본 이므로 다른 참조.
...
const copyObj = { ...obj };
console.log(copyObj); // {name: '아이유', gender: 'woman'}
...
console.log(whopper); // {base: 'meat parry', company: '버거킹', name: '와퍼'}
...
const details = ['아이유', 30];

// spread 문법으로 배열의 개별 요소를 인자로 펼쳐서 전달.
Details(...details); // name : 아이유, age: 30
// 호출"하는" 함수의 파라미터에 사용
...
// 호출"받는" 함수의 파라미터에 사용,
// 호출하는 함수의 파라미터 순서에 맞춰 값 설정 후 남은 파라미터 값을 '배열'로 설정

/* 
    얕은 복사 vs 깊은 복사
    - 객체나 배열과 같은 참조 타입 데이터를 처리할때, 데이터를 복사하는 두가지 주요 방식.
