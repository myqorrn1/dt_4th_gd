...
// - 이 반복은 배열의 요소를 "직접 참조"하므로 인덱스를 사용할 필요가 X
// Ex)
...
// **"각 요소가 함수의 인자로 전달된다"**는 것은 forEach가 배열의 요소를 하나씩 가져와, 그 요소를 지정된 함수의 첫 번째 인자로 전달한다는 뜻입니다.
// 이를 통해 배열의 모든 요소를 반복적으로 처리할 수 있습니다.
const alphabets2 = ['a', 'b', 'c', 'd'];

alphabets2.forEach(function (alpha) {
    // alpha: currentValue를 의미. = 반복하고 있는 현재 요소
    console.log('Element: ', alpha);
});

alphabets2.forEach(function (alpha, idx) {
    // alpha: currentValue를 의미. = 반복하고 있는 현재 요소
    // idx: currentValue의 인덱스 (위치)
    console.log(`Element: ${alpha}, Index: ${idx}`);
});

alphabets2.forEach(function (alpha, idx, arr) {
    // alpha: currentValue를 의미. = 반복하고 있는 현재 요소
    // idx: currentValue의 인덱스 (위치)
    // arr: forEach를 호출한 배열
...
/* 
콜백 함수: function (num) { return num * 2; }
역할: map이 배열의 각 요소를 순회할 때 호출되어, 배열 요소를 변환한 결과를 반환합니다.
콜백 함수와 map의 관계: map 메서드가 콜백 함수의 호출을 책임집니다.
 */
